...
const registerWebViewElement = function () {
  ...
  const createNonBlockHandler = function (method) {
    return function (...args) {
      ...
      const requestId = callback ? getNextId() : null
      ipcRenderer.send('ELECTRON_BROWSER_ASYNC_CALL_TO_GUEST_VIEW', requestId, internal.guestInstanceId, method, ...args)
      if (callback) {
        ipcRenderer.once(`ELECTRON_RENDERER_ASYNC_CALL_TO_GUEST_VIEW_RESPONSE_${requestId}`, (event, result) => callback(result))
      }
    }
  }
  ...
  proto.getWebContents = function () {
    ...
    if (remote) {
      ...
      throw new Error('getWebContents requires remote, which is not enabled')
    }
  }
